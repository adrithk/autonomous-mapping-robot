...
- The ESP32 has separate `keyboard` and `ros_serial` builds with shared encoder-feedback wheel control. The Pi-side ROS package lives at `ros_ws/src/my_bot`, including the serial plugin, simulation and SLAM launch. Initial physical mapping and saved-map navigation are recorded; calibrated odometry, repeated mission acceptance and physical fault-response measurements remain pending. Autonomous exploration is outside the current demonstrated scope.
...
- Preserve the separate keyboard firmware build and select PlatformIO environments explicitly when building or flashing. `src/main.cpp` and `src/main_ros.cpp` are separately filtered PlatformIO entry points; never compile both Arduino `setup()`/`loop()` definitions together. See decision 003 and active plan 003.
